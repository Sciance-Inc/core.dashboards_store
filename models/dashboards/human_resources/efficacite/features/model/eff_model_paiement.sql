...
with
    ...
    y_long as (
        ...
        join
            {{ ref("fact_resultat_etape_competence") }} as cpt
            on fgj.fiche = cpt.fiche
            and fgj.annee = cpt.annee
        where
            cpt.etape = 'EX'  -- Ministère.
            and cpt.code_matiere in ('FRA400', 'FRA600', 'MAT600')
            and cpt.no_comp != '3'
            and cpt.res_etape_num is not null
        group by fgj.lieu_jumele, fgj.annee, cpt.code_matiere
    ...
    y_wide as (
        select
            ...
            -- n_attempts
            max(
                case when code_matiere = 'FRA400' then n_attempts else null end
            ) as n_attempts_fra_q,
            max(
                case when code_matiere = 'FRA600' then n_attempts else null end
            ) as n_attempts_fra_s,
            max(
                case when code_matiere = 'MAT600' then n_attempts else null end
            ) as n_attempts_mat_s,
            -- n_success
            max(
                case when code_matiere = 'FRA400' then n_success else null end
            ) as n_success_fra_q,
            max(
                case when code_matiere = 'FRA600' then n_success else null end
            ) as n_success_fra_s,
            max(
                case when code_matiere = 'MAT600' then n_success else null end
            ) as n_success_mat_s
        from y_long

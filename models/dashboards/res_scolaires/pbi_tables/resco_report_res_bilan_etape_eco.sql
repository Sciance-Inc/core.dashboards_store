...
{{
    config(
        alias="report_res_bilan_etape_eco",
    )
}}
...
with
    data as (
        ...
        where
            ...
            and etape.res_num_comp is not null
    ),
    ...
    agg as (
        select
            ...
            count(res_num_comp) over (
                partition by annee, code_ecole, mat, no_comp, population
            ) as n_obs_g,
            count(res_num_comp) over (
                partition by
                    annee, code_ecole, mat, no_comp, population, plan_interv_ehdaa
            ) as n_obs_pi,
            count(res_num_comp) over (
                partition by annee, code_ecole, mat, no_comp, population, genre
            ) as n_obs_gre,
    ...
    ecart as (
        ...
        from stats
        left join
            {{ ref("resco_report_res_bilan_etape_css") }} as stcss

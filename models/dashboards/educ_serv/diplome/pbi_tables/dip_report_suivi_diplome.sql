    ...
    src_res_mat as (
        ...
        where
            ...
            and (
                ...
                or (
                    res_mat.annee = {{ core_dashboards_store.get_current_year() }}  -- pour le suivi en cours d'année pour les matières avec des épreuves uniques on prend les résultats du bilan jusqu'en juin
                    and month(getdate()) not in (7, 8)
                )  -- pour le résultats finaux on récupère les résultats ministériels, à l'étape suivante, quand ils vont être disponible (mois 7 et 8)    

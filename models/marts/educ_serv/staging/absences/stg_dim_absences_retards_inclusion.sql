...
{#
    ...
    We use the cpt_abs column to distinguish absences from lateness.
    The mapping is described in the stored-Proc [dbo].[GPI_UPDATE_CPT_ABS_MAT_ABS] 

    By default, we split between absences and lateness.
...
group by
    ...
    cpt_abs  -- Test hook. Schould be a dummy aggregation. A test will raise an error if a motif_abs belongs to more than one absence type

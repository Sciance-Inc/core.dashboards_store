...
with adr as (
    select    
        ...
        , cast(adr.date_fin as date) as date_fin
        , adr.ind_envoi_meq 
        , adr.ind_envoi_doc   
        , adr.ind_envoi_trsp                        
        , dim.code_post   
        , row_number() over (partition by adr.fiche, adr.date_effect order by adr.ind_envoi_meq desc, adr.ind_envoi_trsp desc, adr.ind_envoi_doc desc) as seqid   -- si 2 adresses, on privilegie celle qui est envoyée au meq, transport, doc
...
-- conserver les dates min / max durant lesquelles le code postale ne change pas
), adr2 as (
    select distinct
        fiche
        , type_adr
        , code_post
        , min(date_effect) as date_effect
        , max(date_fin) as date_fin
    from adr
    where seqid=1
    group by  fiche, type_adr, code_post
...
-- identifier les annees scolaire d'appartenance de chaque CP
    ...
    select 
	    ...
	    , case when  month(date_fin) < 9 then year(date_fin) - 1 else year(date_fin)
...
select
    ...
    --, y_sco.date_fin
    , y_sco.code_post
    , case when datefromparts(long.annee, 9, 30) between y_sco.date_effect and y_sco.date_fin then 1 else 0 end as adresse_30sept

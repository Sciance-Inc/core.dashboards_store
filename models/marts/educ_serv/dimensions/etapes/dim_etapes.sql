...
with with_model as (
    ...
    join
        ...
        and (mee1.date_deb >= org_year.date_deb and mee1.date_fin <= org_year.date_fin)
...
-- Handle cases were no 'model_etape' is defined
    ...
    from {{ ref("i_gpm_t_org_annee") }} as org_year
    join {{ ref("i_gpm_t_eco") }} as eco
        on 
            org_year.org = eco.org and
            org_year.annee = eco.annee and
            eco.indic_eco_bidon is null 
    join {{ ref('i_gpm_t_etape') }} as etape
        on 
            etape.id_eco = eco.id_eco and
            (
                etape.date_deb >= org_year.date_deb and
                etape.date_fin <= org_year.date_fin
            )
    left join {{ ref('i_gpm_t_modele_etape_client') }} as model_etape_client 
        on 
            model_etape_client.modele_etape is null and -- Make sure we are only fetching the ones without a model
            model_etape_client.id_eco = eco.id_eco
) 

...
::alert{type="warning"} 
This section is only relevant to thoose of the Schoolboards having the Montérégie-Estrie running their ETLs. For the others looking to automate the ETL, please refer to the [production checklist](/using/production)
::
...
cssxx_profile_name:
  ...
  outputs:
    dev:
      ...
      database: <the target database you want the data to be materialized in >
      schema: dbo
...
## How do I run my project now that I have two profiles.yml files?
You will need to specify the path of your profiles.yml that you want to run.
```bash
dbt run --profiles-dir ~/.dbt/
```

Originally, the profiles.yml is located in your **.dbt** folder. If you moved the file, you will have to specify the new path in your dbt run command.
```bash
dbt run --profiles-dir "The_path_of_your_profiles.yml"
```
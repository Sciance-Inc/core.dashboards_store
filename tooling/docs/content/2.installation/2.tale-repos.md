...
::alert{type="warning"}
When integrating the Store into your school board, all modifications you make (overriding, enabling, implementing an adapter, populating a seed) should take place from inside the `cssXX.dashboards_store` repo. The `core.dashboards_store` repo should never be modified as it is shared between all school boards.
::
...
4. Let's follow best practices and use Git for the repo you have just created
...
5. Read the `cssXX.dashboards_store/README.md` to learn more about the post-configuration steps required to have everything working.
...
::alert{type="info"}
From now on, `<working directory>` will always designate the path to the folder containing the two repos (unless mentioned otherwise).
::

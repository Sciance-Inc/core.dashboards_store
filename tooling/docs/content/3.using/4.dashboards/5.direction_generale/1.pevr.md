...
Les bases de données `gpi` et `jade` doivent être liées au projet. Veuillez consulter la section [lier une base de données](/using/configuration/databases) pour plus d'informations sur la manière de lier une base de données.
...
Si le concept d'`overriding` est nouveau pour vous, veuillez lire [cette page](/using/configuration/overriding).
::

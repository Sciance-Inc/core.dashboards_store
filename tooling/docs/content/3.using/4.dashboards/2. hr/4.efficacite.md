...
>>>>>>> 950e867 (doc(efficacite): ajout de la documentation)
=======
> Pour peupler la seed `eff_coefficients_premier_axes`, vous devez exécuter le script python situé dans le répertoire `core.dashboards_store/analyses/Dashboard_efficacite_extract_coeff.py`. Assurez-vous par la suite d'inscrire les données dans les bonnes colonnes dans la seed.
>>>>>>> a1b0cb8 (doc(efficacite): paufinement de la doc)
=======
> Pour peupler la seed `eff_coefficients_premier_axes`, vous devez exécuter le script python situé dans le répertoire `core.dashboards_store/analyses/efficacite/dashboard_efficacite_extract_coeff.py`. Assurez-vous par la suite d'inscrire les données dans les bonnes colonnes dans la seed.
>>>>>>> 6f6cb2d (feat(efficacite): adding script for computing coefficients)
...
<<<<<<< HEAD
<<<<<<< HEAD
Si la colonne lieu_jumelé n'est pas identique à la colonne lieu_trav dans la seed `eff_mapping_fgj_paie`, nous recommandons fortement d'écrire le nom de l'école du lieu_jumelé avec la catégorie de l'école. Selon l'exemple décrite ci-dessus pour eff_mapping_fgj_paie:
=======

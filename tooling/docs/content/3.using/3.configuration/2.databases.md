...
::alert{type="danger"}
Databases accessed by the Store / ETL should be pseudo-anonymized. It's not mandatory, but it's highly recommended, as analytical workloads usually do not require the use of personal information ;)
::

# How to link the databases ?
> The `cssXX.dashboards_store/dbt_project.yml` exposes a `databases` section in which you will populate the database names of the databases you want to link the Store to.

Basically, the process to add a database goes as follows:
1. Open the `cssXX.dashboards_store/dbt_project.yml` file.
2. Replace the database placeholder with the database **Fully Qualified Name**.

...
Every folder that contains one or more SQL models must contain its own `schema.yml` file.

The rule is deliberately local:

```text
one folder with .sql files
-> one schema.yml in that same folder
-> only the .sql models from that folder
```

A `schema.yml` file must not document a model located in a subfolder or parent folder. When a dashboard contains folders such as `features`, `dimensions`, `staging`, or `pbi_tables`, each of those folders must carry the documentation for the models it contains.

Every SQL model must have a `models:` entry in the `schema.yml` file from its own folder. Conversely, no `models:` entry should point to a model that is absent from the current folder.

`adapters.yml` files are the exception: they document contracts that `cssXX.dashboards_store` repos must implement and do not follow this rule. Contractual seeds documented in the `core`, but implemented by other repos, must also remain documented even when no local `.csv` file exists in the `core`.
...
> This dashboard requires the specification of the seeds from the `human_resources` mart.
...
Dashboard-specific seeds should instead be defined and documented under `seeds/dashboards/<domain>/<dashboard>/`.

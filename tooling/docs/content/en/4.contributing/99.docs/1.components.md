...
::code-group
  ...
  ::code-block{label="Preview" preview}
    ...
    Check out an **info** alert with `code` and a [link](/en/).
    ::

    ::alert{type="success"}
    Check out a **success** alert with `code` and a [link](/en/).
    ::

    ::alert{type="warning"}
    Check out a **warning** alert with `code` and a [link](/en/).
    ::

    ::alert{type="danger" style="margin-bottom: 0;"}
    Check out a **danger** alert with `code` and a [link](/en/).
    ::
  ::
...
`<Badge />` supports the same types as `<Alert />`.
...
`<Callout />` supports the same types as `<Alert />`.
...
This component uses `slots` to create a tab panel of your code examples or previews.
...
The Icon component gives you access to all **100,000+** icons from [icones.js.org](https://icones.js.org).
...
Embed CodeSandbox/StackBlitz easily in your documentation with great performance.

It uses the [IntersectionObserver](https://developer.mozilla.org/en-US/docs/Web/API/Intersection_Observer_API) to load only when visible in the viewport.

...
We assume you have a terminal open in the `<working dir>` folder.
...
3. Run the debug command to test the connection:
...
::alert{type="success"}
Yay! You are now ready to populate, configure and run the ETL!
::
...
- If you get an error, obviously something is not working:
  ...
  - Make sure your database is actually reachable from your computer (try `telnet <database's IP> <database's port>`)

...
1. Import the `users` table into your Power BI dashboard, and create a relationship between the `users` table and the reporting tables. **The join must be done on the `school` column**. This is the big deal: **if your reporting table DOES NOT contain a `school` column, you won't be able to join the `user_table` table, and you won't be able to apply RLS to this table**.
2. Create a role :
   1. In Power BI Desktop, click on the `Model` tab, then click on `Manage Roles`.
   2. Click on `Create` to create a new role.
   3. In the `Manage roles` dialog box, enter a name for the role, and then click on `Add Table Filter`.

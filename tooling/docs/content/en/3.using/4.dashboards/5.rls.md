...
::alert{type="danger"}
This page is a draft. It's something you will have to decipher until I find the time to complete it :(
::
...
## What you will need ? 
::alert{type="info"}
The 'users table' must be implemented following your own business logic. In this example, we assume that user identification is done through the **email address** people use to connect to Power BI. If you are using an alternative way to authenticate people, you will have to adapt this example.
::
...
- `user_id` : the unique identifier of the user, which can be mapped back to an item of the Power BI authentication payload.
...
1. Ensure that your data model has the necessary columns to define RLS filters. Basically, you need a set of dimensions you want to apply RLS to, to be available in both the reporting table and the `user_table`. The `user_table` should also include a column acting as a primary means of identification for the user (in the example, we will be using the `email` column). 
2. Import the `user_table` into your Power BI dashboard, and create a relationship between the `user_table` and the reporting tables. **The join must be done on the scope column**.
3. Create and manage a role:
...
1. Import the `users` table into your Power BI dashboard, and create a relationship between the `users` table and the reporting tables. **The join must be done on the `school` column**. This is the big deal: **if your reporting table DOES NOT contain a `school` column, you won't be able to join the `user_table`, and you won't be able to apply RLS to this table**.
2. Create a role :
   1. In Power BI Desktop, click on the `Model` tab, then click on `Manage Roles`.
   2. Click on `Create` to create a new role.
   3. In the `Manage roles` dialog box, enter a name for the role, and then click on `Add Table Filter`.
...
1. The user logs into Power BI with their email, and the report is displayed.
2. Power BI sets `USERPRINCIPALNAME()` to the user's email.

...
::alert{type=warning}
When shipping to production (and staging), always pin the version of the `core.dashboards_store` your `cssXX.dashboards_store` depends on.
::
...
You can build a CI/CD pipeline to automate the deployment of your dashboards. The `core.dashboards_store/tooling/depechecode/Dockerfile` describes a Docker image packaging the core you can leverage to build the image for your `cssXX.dashboards_store`.

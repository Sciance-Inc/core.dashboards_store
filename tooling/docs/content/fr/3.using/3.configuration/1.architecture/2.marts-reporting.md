...
La Bibliothèque de tableaux de bord (le *Dashboards Store*) ne demande pas à Power BI d'être l'entrepôt analytique.
...
Il sert à définir une information une fois, avec un grain clair, des clés stables et des dimensions réutilisables. Une table de *mart* doit pouvoir servir plusieurs usages, plusieurs pages ou plusieurs tableaux de bord.
...
La couche de *reporting* prépare les tables finales consommées par les tableaux de bord.
...
Le but n'est pas de faire du Kimball pur dans Power BI. Le but est de livrer à Power BI des tables faciles à utiliser, précalculées et stables du point de vue du rapport.
...
Elle doit être promue vers un *mart* lorsqu'elle devient réutilisable, stable ou structurante.
...
```text
Cette métrique doit-elle devenir une définition commune de la Bibliothèque?
```
...
Ces tables servent à synchroniser les filtres entre les pages, à contrôler les combinaisons valides et à éviter de répéter les mêmes colonnes de filtre partout.

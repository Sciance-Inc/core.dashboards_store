...
- Les *dashboards* déployés sont versionnés dans `cssXX.dashboards_store`.
- Un *push* vers `cssXX.dashboards_store` déclenche une *pipeline* CI/CD qui construit une image Docker basée sur celle de `core.dashboards_store`. Mon *ETL* est conteneurisé.

...
::alert{type=success}
Si vous avez créé votre Store avec le *template* `cookiecutter`, les dossiers `cssXX.dashboards_store/models/marts/educ_serv/staging/` contiennent déjà des *adapters* préremplis pour vous aider à démarrer. Vous devez seulement mettre à jour les fichiers `sql` avec vos propres règles d'affaires.
::
